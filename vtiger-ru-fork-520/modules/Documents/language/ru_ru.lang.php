<?php
/***********************************************************
*  Module       : Documents
*  Language     : RU Русский
*  Version      : 5.2.0
*  Created Date : 2010-08-23
*  Author       : Eugene Babiy
*  License      : 
Licensed under Vtiger Public License Version 1.1.

If you have a questions or suggestions about this translation module, please contact me.

Если у Вас возникли вопросы или замечания касающиеся этого перевода, пожалуйста свяжитесь со мной любым из удобных для Вас способов.

e-mail: eugene.babiy@gmail.com
mob: +380660785088
Skype: eugene.babiy
***********************************************************/


$mod_strings = Array(
'LBL_MODULE_NAME'=>'Документы',
'LBL_MODULE_TITLE'=>'Документы: Главная',
'LBL_SEARCH_FORM_TITLE'=>'Поиск Документа',
'LBL_LIST_FORM_TITLE'=>'Список Документов',
'LBL_NEW_FORM_TITLE'=>'Новый Документ',

'LBL_LIST_SUBJECT'=>'Тема',
'LBL_LIST_CONTACT_NAME'=>'Контакт',
'LBL_LIST_RELATED_TO'=>'Относится к',
'LBL_LIST_DATE_MODIFIED'=>'Последнее Изменение',

'LBL_NOTE'=>'Документ:',
'LBL_NOTE_SUBJECT'=>'Тема Документа:',
'LBL_CONTACT_NAME'=>'Название Контакта:',
'LBL_PHONE'=>'Телефон:',
'LBL_SUBJECT'=>'Тема:',
'LBL_CLOSE'=>'Закрыто:',
'LBL_RELATED_TO'=>'Относится к:',
'LBL_EMAIL_ADDRESS'=>'Адрес Email:',
'LBL_COLON'=>':',

'ERR_DELETE_RECORD'=>"Необходимо указать номер записи для удаления vtiger_account.",
'LBL_LIST_FILENAME'=>"Название Файла",

// Added for Note(3.2-patch 3) 

'LBL_FILENAME'=>'Вложение:',
'LBL_NOTE_INFORMATION'=>'Базовая Информация',
'File Information'=>'Информация Файла',

// Added for 4GA
'LBL_TOOL_FORM_TITLE'=>'Инструменты Документов',
// Added for 4GA
'Contact Name'=>'Контакт',
'Related To'=>'Относится к',
'Subject'=>'Тема',
'Created Time'=>'Время Создания',
'Modified Time'=>'Время Изменения',
'Document'=>'Документ',
'Note'=>'Замечания',
//Added for 4.2 Release -- CustomView
'Related to'=>'Относится к',
'Last Modified'=>'Последним Изменил',
'File'=>'Файл',
'LBL_ALL'=>'Все',
'Title'=>'Оглавление',

//Added after 504 for Downloads Module
'LBL_FILE_INFORMATION'=>'Информация Файла',
//'LBL_FILE_NAME'=>'Название Файла',
'LBL_FILE_TYPE'=>'Тип Файла',
'LBL_FILE_SIZE'=>'Размер',
'filelocationtype'=>'Размещение',
'fileversion'=>'Версия',
'filestatus'=>'Активен',
'filedownloadcount'=>'Счетчик Загрузок',
'os'=>'Платформа',
'LBL_FOLDER_NAME'=>'Название Папки',
'LBL_DOWNLOAD_TYPE'=>'Размещение',
'LBL_INTERNAL'=>'База Данных',
'LBL_EXTERNAL'=>'Внешнее',
//'LBL_EXTERNAL_FILE_NAME'=>'Название Файла',
'LBL_FILE_LOCATION'=>'Размещение Файла',
'LBL_PD'=>'Платформозависимый',
'LBL_PIND'=>'Платформонезависимый',
'ChoosePlatform'=>'Выбрать Платформу',
'LBL_ACTIVE'=>'Активен',
'LBL_INACTIVE'=>'Неактивен',
'LBL_UPLOAD_BUTTON'=>'Выгрузить',
'architecture'=>'Архитектура',
'LBL_DOWNLOAD_FILE'=>'Загрузить файл',
'LBL_CHECK_INTEGRITY'=>'Проверить целостность файла',
'LBL_EMAIL_FILE'=>'Отправить на Email',
'LBL_CLICK_HERE'=>'Нажмите здесь',
'LBL_TO_ADD_FOLDER'=>'чтобы добавить новую папку.',
'LBL_DELETE_FOLDER'=>'Удалить Папку',
'LBL_FOLDER_DESC'=>'Описание',
'LBL_ADD_NEW_FOLDER'=>'Добавить Папку',
'LBL_DELETE_DOCUMENTS'=>'Удалить Документ(ы)',
'LBL_MOVE_TO'=>'Переместить в',
'File Name'=>'Название Файла',
'File Type'=>'Тип Файла',
'File Size'=>'Размер',
'LBL_INTERNAL'=>'Внутри',
'LBL_URL'=>'URL',
'LBL_EXTERNAL'=>'Внешний',
'LBL_EXTERNAL_LNK'=>'Внешняя Ссылка',
'LBL_ACTIVE'=>'Активен',
'LBL_INACTIVE'=>'Неактивен',
'LBL_PLATFORM_INDEPENDENT'=>'Независимая Платформа',
'LNK_DOWNLOAD'=>'загрузить',
'LBL_MOVE_DOCUMENTS'=>'Переместить Документ(ы)',
'LBL_MOVE'=>'Переместить',
'Download Type'=>'Размещение',
'Version'=>'Версия',
'Active'=>'Активен',
'Download Count'=>'Счетчик Загрузок',
'Platform'=>'Платформа',
'Assigned To'=>'Ответственный',
// Module Sequence Numbering
'Note No' => 'Документ №',
// END

// Documents module
'LBL_EMPTY_FOLDERS' => 'Пустые Папки',
'LBL_VIEW_EMPTY_FOLDERS' => 'Просмотр Пустых Папок',
'LBL_NO_DOCUMENTS' => 'Нет Документов',
'LBL_DESCRIPTION' => 'Описание',
'FILE_HAS_NO_DATA'=>'Выбранный файл не имеет данных или поврежден',

'LBL_MAXIMUM_20'=>'Максимум 20',
'LBL_MAXIMUM_50'=>'Максимум 50',

// Added according to Ticket #6198
'Folder Name'=>'Название Папки',
'Document No'=>'Документ №',
'Internal'=>'База Данных',
'External'=>'Внешнее',
);
?>
