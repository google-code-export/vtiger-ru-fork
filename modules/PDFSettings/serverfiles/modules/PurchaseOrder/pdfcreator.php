...
function createpdffile ($idnumber,$purpose='', $path='',$current_id='') {
	...
	//select language file
		switch($pdf_config_details[pdflang])
		{
			case "en_us":
				...
				break;
			case "ge_de":
				include("modules/PurchaseOrder/language/ge_de.lang.pdf.php");
				$language = "DE";
				break;
			default:
				include("modules/PurchaseOrder/language/en_us.lang.pdf.php");
	...
	// send pdf with mail
		...
		{
			case "EN":
				$pdf->Output('storage/PurchaseOrder_'.$_REQUEST['record'].'.pdf','F'); //added file name to make it work in IE, also forces the download giving the user the option to save
				break;
			case "DE":
				$pdf->Output('storage/Einkaufsbestellung_'.$_REQUEST['record'].'.pdf','F'); //added file name to make it work in IE, also forces the download giving the user the option to save
				break;
		}
		return;
	}
