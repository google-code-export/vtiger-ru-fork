...
if(isPermitted("Issuecards","EditView",$_REQUEST['record']) == 'yes')
	$smarty->assign("EDIT_DUPLICATE","permitted");

$smarty->assign("CREATEPDF","permitted");

if(isPermitted("Issuecards","Delete",$_REQUEST['record']) == 'yes')
	$smarty->assign("DELETE","permitted");

//Get the associated Products and then display above Terms and Conditions
$smarty->assign("ASSOCIATED_PRODUCTS",getIssuecardsDetailAssociatedProducts('Issuecards',$focus));

$check_button = Button_Check($module);
$smarty->assign("CHECK", $check_button);

$tabid = getTabid("Issuecards");
...
$smarty->display("modules/Issuecards/InventoryDetailView.tpl");


function getIssuecardsDetailAssociatedProducts($module,$focus)
{
	global $log;
	$log->debug("Entering getIssuecardsDetailAssociatedProducts(".$module.",".get_class($focus).") method ...");
	global $adb;
	global $mod_strings;
	global $theme;
	global $log;
  ...
  $taxtype = getIssuecardsInventoryTaxType($focus->id);
	...
	$currencytype = getIssuecardsInventoryCurrencyInfo($focus->id);
